...
public class DataParserUtil {
    ...
    public BLEDataContent parse(byte[] data) {
        if (data.length < 3) {
            // TODO LogManager.getInstance().warn("Data received from device has invalid length!");
            return null;
        }
        ...
        switch (ConverterUtil.dataToInt(data[0])) {
            ...
            default:
                ...
                // TODO LogManager.getInstance().warn("Unknown data protocol received!");
                return null;
        }
    }
    ...
    private BLEDataContent parseForProtocolV1(byte[] data) {
        // TODO LogManager.getInstance().error("Protocol V1 is ignored to process response!");
        return null;
    }

    private BLEDataContent parseForProtocolV2(byte[] data) {
        byte[] groupData = Arrays.copyOfRange(data, 1, data.length);

        switch (data[0]) {
            case PacketHeaders.PROTOCOLV2.GROUP.AUTH:
                byte[] typeData = Arrays.copyOfRange(data, 1, data.length);
